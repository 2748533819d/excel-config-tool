...
开启 SXSSF 流式写入（配置 `"streaming": true`）可大幅降低内存占用：

| 对比维度 | XSSF（默认） | SXSSF 流式 | 改善倍数 |
|---------|------------|-----------|---------|
| 10K×10 堆内存增量 | 355 MB | **38 MB** | **~9x** |
| 100K×10 堆内存增量 | 1,806 MB | **108 MB** | **~17x** |
| 100K×10 执行耗时 | 10,843 ms | **1,154 ms** | **~9x** |
| 最低堆要求 | -Xmx3G | **-Xmx512M** | **~6x** |
